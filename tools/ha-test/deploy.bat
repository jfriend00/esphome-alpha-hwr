...
REM /XD profiles results __pycache__ excludes the runtime "profiles" folder from copy/purge so saved
REM profiles are never touched by a deploy (even if /MIR is ever re-enabled).
if defined DRYRUN (
    REM robocopy "%SRC%" "%DEST%" /MIR /XD profiles results __pycache__ /L
    robocopy "%SRC%" "%DEST%" /E /XD profiles results __pycache__ /L
) else (
    REM robocopy "%SRC%" "%DEST%" /MIR /XD profiles results __pycache__ /NFL /NDL /NJH /NJS /NP
    robocopy "%SRC%" "%DEST%" /E /XD profiles results __pycache__ /NFL /NDL /NJH /NJS /NP
)
set "RC=%ERRORLEVEL%"
