...
// Reference: alpha_hwr/constants.py::CRC_TABLE
...
uint16_t calc_crc16(const uint8_t *data, size_t len, uint16_t init) {
  uint16_t crc = init;
...
// CRC-16-CCITT calculation with final XOR (used for READ/INFO/EXECUTE operations)
// Reference: alpha_hwr/utils.py::calc_crc16_read()
uint16_t calc_crc16_read(const uint8_t *data, size_t len, uint16_t init) {
  return calc_crc16(data, len, init) ^ 0xFFFF;
}
